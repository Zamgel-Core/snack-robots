...
export function Roles() {
  ...
  async function loadAccess() {
    ...
    try {
      const data = await getCurrentAccess(user.id, profile?.global_role);
      setAccess(data);
    } catch (err) {
      console.error(err);
      setError('No se pudo cargar la información de roles. Revisa permisos de store_members.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-8 pb-10 text-white">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-xl font-medium text-blue-200">
            Define qué puede hacer cada persona y protege acciones críticas con autorización de Super Admin.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-black text-blue-900">
              El Store Admin puede operar casi todo, pero acciones delicadas deben pedir PIN de Super Admin, como en sistemas comerciales reales.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        ...
        <StatCard label="Nivel" value={roleInfo.badge} icon={BadgeCheck} />
        <StatCard label="PIN requerido" value="Acciones críticas" icon={KeyRound} />
        <StatCard label="Modelo" value="3 roles base" icon={UsersRound} />
      </div>
        ...
        <div className="mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-bold text-blue-200">
              Primera versión funcional para identificar accesos. Después conectamos bloqueo real por ruta y acción.
            </p>
          </div>
        </div>
          ...
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-bold text-yellow-100/80">
                El Store Admin puede operar con poder avanzado, pero estas acciones deben tener autorización superior.
              </p>
            </div>
          </div>
        ...
        <div className="rounded-[32px] border border-white/20 bg-white/10 p-6 shadow-2xl backdrop-blur-md">
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-bold text-blue-200">Este módulo queda preparado para usuarios reales.</p>
            </div>
          </div>

          <div className="space-y-3">
            {[
              'Invitar empleados por correo.',
              'Asignar rol por tienda.',
              'Bloquear rutas según permisos.',
              'Pedir PIN cuando Store Admin haga acciones críticas.',
              'Crear auditoría de quién hizo cada cambio.',
            ].map((item) => (
