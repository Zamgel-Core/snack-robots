  ...
  if (loading) {
    return (
      <div className="text-white font-black text-xl">
        Cargando datos reales de Snack Robots...
      </div>
    );
  }
      ...
      <header className="flex justify-between items-end mb-6">
        <div>
          ...
          <p className="text-xl text-blue-100 font-medium">
            Aquí está el resumen real de tu negocio hoy.
          </p>
        </div>

        <div className="hidden md:flex gap-3">
          <div className="bg-white/10 backdrop-blur-md px-5 py-2 rounded-2xl border border-white/20 text-right">
            <p className="text-[10px] uppercase font-bold text-yellow-300">
              Status de Caja
            </p>
            <p className="text-lg font-black text-white">PENDIENTE</p>
          </div>
        </div>
      </header>
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-white/10 backdrop-blur-md rounded-[32px] border border-white/20 p-8 flex flex-col shadow-xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="bg-yellow-400 text-blue-900 text-xs px-3 py-1 rounded-full font-black uppercase tracking-widest">
                En progreso
              </span>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end mb-2 text-white">
                <div>
                  <p className="font-bold text-blue-200 uppercase text-sm mb-1">
                    Llegar a $10.00 vendidos
                  </p>
              ...
              <p className="text-sm font-semibold text-yellow-300 text-center mt-4">
                ¡Faltan {formatCurrency(Math.max(0, 10 - todaySales))} para la
                meta!
              </p>
            </div>
          </section>
          ...
          <RobotTip
            message="Robo Blue está leyendo datos reales de Supabase. Cuando registres ventas, aquí aparecerán tus ganancias del día."
            type="info"
          />
        </div>
          ...
          <section className="bg-black/20 backdrop-blur-md rounded-[32px] p-6 border border-white/10 shadow-xl h-full flex flex-col text-white">
            ...
            <div className="space-y-4 flex-grow">
              {outOfStock.map((p: any) => (
                <div
                  key={p.id}
                  className="p-4 bg-red-500/20 border border-red-500/30 rounded-2xl flex items-center justify-between"
                >
                  <span className="font-bold text-red-100">{p.name}</span>
                  <span className="px-3 py-1 bg-red-500 text-white rounded-full text-[10px] font-black uppercase">
                    Agotado
                  </span>
                </div>
              ))}
              ...
              {lowStock.map((p: any) => (
                <div
                  key={p.id}
                  className="p-4 bg-orange-400/20 border border-orange-400/30 rounded-2xl flex items-center justify-between"
                >
